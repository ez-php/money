...
- **`MoneyFormatter` / `MoneyParser` are interfaces without the `Interface` suffix** — unlike the rest of the monorepo (`CacheInterface`, `ExchangeRateProviderInterface`, …). The names read as roles at call sites (`MoneyFormatter $formatter`) and were published that way; renaming them would break every implementation and type hint, so the deviation is kept deliberately until a major version bundles the rename. New interfaces in this module use the `Interface` suffix.
